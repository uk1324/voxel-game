...
void RenderSystem::update(const EntityManager& entityManager, Entity player, const Mat4& view, const Mat4& projection)
{
	...
	m_shader.setVec3("lookDir", entityManager.getComponent<Rotation>(player).value * Vec3(0, 0, 1));
	...
	{
		const auto& position = entityManager.getComponent<Position>(entity).value;
		const auto& rotation = entityManager.getComponent<Rotation>(entity).value;
		...
		//Primitives::cubeTrianglesVao->bind();
		//drawTriangles(0, 36);
